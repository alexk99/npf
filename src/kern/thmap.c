...
{
	const uint32_t hashval = murmurhash2(key, len, 0);
...
hashval_getslot(thmap_query_t *query, const void * restrict key, size_t len)
{
	...
	if (query->hashidx != i) {
		...
		query->hashval = murmurhash2(key, len, i);
		query->hashidx = i;
	}
    ...
    const thmap_leaf_t *leaf, unsigned level)
{
	...
	return (murmurhash2(key, leaf->len, i) >> shift) & LEVEL_MASK;
}
